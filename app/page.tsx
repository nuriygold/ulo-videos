...
export default function Home() {
  ...
  const speechAvailable = setup.renderer.capabilities.speech;
    ...
    <div className="workspace-grid">
      <section className="editor" aria-labelledby="editor-title">
        ...
        <form className="scene-form" onSubmit={onSaveShot}>
          ...
          <fieldset disabled={!project || busy !== null}><legend>Spokescharacter</legend><Field label="Character .blend, .gltf, .glb, or .fbx file" container><div className="file-row"><label className="file-picker"><span>{characterFile?.name || (draft.characterAsset ? "Uploaded character" : "Choose a character file")}</span><input className="file-input" type="file" accept=".blend,.gltf,.glb,.fbx,application/x-blender,model/gltf+json,model/gltf-binary,application/octet-stream" onChange={(e) => setCharacterFile(e.target.files?.[0] || null)} required={!draft.characterAsset && !characterFile} /></label><button type="button" className="secondary demo-button" onClick={() => loadDemo("character")}>Load demo</button></div></Field>{characterFile&&<small className="file-ready">✓ Ready to upload: {characterFile.name}</small>}<div className="fields three"><Field label="Position"><select value={draft.position} onChange={(e) => update("position",e.target.value as InterruptionDraft["position"])}><option value="foreground_left">Left</option><option value="foreground_center">Center</option><option value="foreground_right">Right</option></select></Field><Field label="Entrance"><select value={draft.entrance} onChange={(e) => update("entrance",e.target.value as InterruptionDraft["entrance"])}><option value="pop_in">Pop in</option><option value="fade_in">Fade in</option><option value="slide_left">Slide left</option><option value="slide_right">Slide right</option></select></Field><Field label="Gesture"><select value={draft.gesture} onChange={(e) => update("gesture",e.target.value as InterruptionDraft["gesture"])}><option value="shrug_and_point">Shrug + point</option><option value="wave">Wave</option><option value="nod">Nod</option><option value="talk_idle">Talk idle</option></select></Field></div><Field label={speechAvailable ? "Voice reference" : "Voice reference (unavailable)"}><input value={draft.voice} onChange={(e) => update("voice",e.target.value)} disabled={!speechAvailable} /></Field><Field label={speechAvailable ? "Lip-sync reference" : "Lip-sync reference (unavailable)"}><input value={draft.lipSync} onChange={(e) => update("lipSync",e.target.value)} disabled={!speechAvailable} /></Field></fieldset>
