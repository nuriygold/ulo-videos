...
import { buildInterruptionScene, createProject, saveShot, submitRender, type InterruptionDraft } from "../src/web/workspace-client";
...
type Project = { id: string; name: string }; type Shot = { id: string; name: string }; type Job = { id: string; status: string; progress: number };
...
export default function Home() {
  ...
  async function onSaveShot(event: FormEvent) {
    ...
    try { setShot(await saveShot(project.id, draft.shotName, buildInterruptionScene(draft))); setJob(null); setNotice("Shot snapshot saved and ready to submit."); }
  ...
  async function onRender() {
    ...
    try { setJob(await submitRender(project.id, shot.id, buildInterruptionScene(draft))); setNotice("Render accepted by the control plane."); }
    ...
    <div className="workspace-grid">
      <section className="editor" aria-labelledby="editor-title">
        ...
        <form className="scene-form" onSubmit={onSaveShot}>
          <fieldset disabled={!project || busy !== null}><legend>Shot and timing</legend><div className="fields two"><Field label="Shot name"><input value={draft.shotName} onChange={(e) => update("shotName",e.target.value)} maxLength={120} required /></Field><Field label="Interrupt at (seconds)"><input type="number" min="0" step="0.1" value={draft.pauseAt} onChange={(e) => update("pauseAt",e.target.value)} required /></Field></div><Field label="Source video Blob URL"><input type="url" value={draft.sourceVideo} onChange={(e) => update("sourceVideo",e.target.value)} placeholder="https://…blob.vercel-storage.com/source.mp4" required /></Field></fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Spokescharacter</legend><Field label="Character asset Blob URL"><input type="url" value={draft.characterAsset} onChange={(e) => update("characterAsset",e.target.value)} placeholder="https://…/character.blend" required /></Field><div className="fields three"><Field label="Position"><select value={draft.position} onChange={(e) => update("position",e.target.value as InterruptionDraft["position"])}><option value="foreground_left">Left</option><option value="foreground_center">Center</option><option value="foreground_right">Right</option></select></Field><Field label="Entrance"><select value={draft.entrance} onChange={(e) => update("entrance",e.target.value as InterruptionDraft["entrance"])}><option value="pop_in">Pop in</option><option value="fade_in">Fade in</option><option value="slide_left">Slide left</option><option value="slide_right">Slide right</option></select></Field><Field label="Gesture"><select value={draft.gesture} onChange={(e) => update("gesture",e.target.value as InterruptionDraft["gesture"])}><option value="shrug_and_point">Shrug + point</option><option value="wave">Wave</option><option value="nod">Nod</option><option value="talk_idle">Talk idle</option></select></Field></div><Field label="Dialogue"><textarea value={draft.dialogueText} onChange={(e) => update("dialogueText",e.target.value)} rows={3} /></Field><div className="fields two"><Field label="Voice reference"><input value={draft.voice} onChange={(e) => update("voice",e.target.value)} required /></Field><Field label="Lip-sync reference"><input value={draft.lipSync} onChange={(e) => update("lipSync",e.target.value)} required /></Field></div></fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Brand and output</legend><Field label="Logo Blob URL"><input type="url" value={draft.logo} onChange={(e) => update("logo",e.target.value)} placeholder="https://…/logo.svg" required /></Field><div className="fields two"><Field label="Caption placement"><select value={draft.captionStyle} onChange={(e) => update("captionStyle",e.target.value as InterruptionDraft["captionStyle"])}><option value="none">None</option><option value="lower_third">Lower third</option><option value="top">Top</option><option value="center">Center</option></select></Field><label className="check"><input type="checkbox" checked={draft.captionsEnabled} onChange={(e) => update("captionsEnabled",e.target.checked)} /><span>Generate captions</span></label></div><div className="fields three"><Field label="Width"><input type="number" min="1" value={draft.width} onChange={(e) => update("width",e.target.value)} required /></Field><Field label="Height"><input type="number" min="1" value={draft.height} onChange={(e) => update("height",e.target.value)} required /></Field><Field label="FPS"><input type="number" min="1" value={draft.fps} onChange={(e) => update("fps",e.target.value)} required /></Field></div></fieldset>
