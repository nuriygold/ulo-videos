    ...
    <div className="workspace-grid">
      <section className="editor" aria-labelledby="editor-title">
        ...
        <form className="scene-form" onSubmit={onSaveShot}>
          <fieldset disabled={!project || busy !== null}><legend>Shot and timing</legend><div className="fields two"><Field label="Shot name"><input value={draft.shotName} onChange={(e) => update("shotName",e.target.value)} maxLength={120} required /></Field><Field label="Interrupt at (seconds)"><input type="number" min="0" step="0.1" value={draft.pauseAt} onChange={(e) => update("pauseAt",e.target.value)} required /></Field></div><Field label="Source video file (uploads directly to Blob)"><div className="file-row"><input type="file" accept="video/mp4,video/quicktime,video/webm" onChange={(e) => setSourceFile(e.target.files?.[0] || null)} required={!draft.sourceVideo} /><button type="button" className="secondary demo-button" onClick={() => loadDemo("source_video")}>Load demo</button></div></Field>{sourceFile&&<small>Selected source: {sourceFile.name}</small>}{draft.sourceVideo&&<small>Uploaded source: {draft.sourceVideo}</small>}</fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Spokescharacter</legend><Field label="Character .blend file (uploads directly to Blob)"><div className="file-row"><input type="file" accept=".blend,application/x-blender" onChange={(e) => setCharacterFile(e.target.files?.[0] || null)} required={!draft.characterAsset} /><button type="button" className="secondary demo-button" onClick={() => loadDemo("character")}>Load demo</button></div></Field>{characterFile&&<small>Selected character: {characterFile.name}</small>}<div className="fields three"><Field label="Position"><select value={draft.position} onChange={(e) => update("position",e.target.value as InterruptionDraft["position"])}><option value="foreground_left">Left</option><option value="foreground_center">Center</option><option value="foreground_right">Right</option></select></Field><Field label="Entrance"><select value={draft.entrance} onChange={(e) => update("entrance",e.target.value as InterruptionDraft["entrance"])}><option value="pop_in">Pop in</option><option value="fade_in">Fade in</option><option value="slide_left">Slide left</option><option value="slide_right">Slide right</option></select></Field><Field label="Gesture"><select value={draft.gesture} onChange={(e) => update("gesture",e.target.value as InterruptionDraft["gesture"])}><option value="shrug_and_point">Shrug + point</option><option value="wave">Wave</option><option value="nod">Nod</option><option value="talk_idle">Talk idle</option></select></Field></div><Field label="Dialogue"><textarea value={draft.dialogueText} onChange={(e) => update("dialogueText",e.target.value)} rows={3} /></Field><div className="fields two"><Field label="Voice reference"><input value={draft.voice} onChange={(e) => update("voice",e.target.value)} required /></Field><Field label="Lip-sync reference"><input value={draft.lipSync} onChange={(e) => update("lipSync",e.target.value)} required /></Field></div></fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Brand and output</legend><Field label="Logo image file (uploads directly to Blob)"><div className="file-row"><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(e) => setLogoFile(e.target.files?.[0] || null)} required={!draft.logo} /><button type="button" className="secondary demo-button" onClick={() => loadDemo("logo")}>Load demo</button></div></Field>{logoFile&&<small>Selected logo: {logoFile.name}</small>}<div className="fields two"><Field label="Caption placement"><select value={draft.captionStyle} onChange={(e) => update("captionStyle",e.target.value as InterruptionDraft["captionStyle"])}><option value="none">None</option><option value="lower_third">Lower third</option><option value="top">Top</option><option value="center">Center</option></select></Field><label className="check"><input type="checkbox" checked={draft.captionsEnabled} onChange={(e) => update("captionsEnabled",e.target.checked)} /><span>Generate captions</span></label></div><div className="fields three"><Field label="Width"><input type="number" min="1" value={draft.width} onChange={(e) => update("width",e.target.value)} required /></Field><Field label="Height"><input type="number" min="1" value={draft.height} onChange={(e) => update("height",e.target.value)} required /></Field><Field label="FPS"><input type="number" min="1" value={draft.fps} onChange={(e) => update("fps",e.target.value)} required /></Field></div></fieldset>
