...
import { buildInterruptionScene, createProject, listProjects, listRenderJobs, saveShot, submitRender, uploadAsset, type InterruptionDraft } from "../src/web/workspace-client";
...
type Project = { id: string; name: string; workspaceId: string }; type Shot = { id: string; name: string }; type Job = { id: string; status: string; progress: number };
...
export default function Home() {
  ...
  useEffect(() => { listProjects().then((items) => { const first = items[0]; if (first) { setProject(first); setNotice("Workspace restored. Choose or edit the saved shot below."); listRenderJobs(first.id).then(setHistory).catch(() => undefined); } }).catch(() => undefined); }, []);
  ...
  return <main className="workspace">
    <header className="hero"><div><p className="eyebrow">Ulo videos · anonymous workspace</p><h1>Build one precise interruption.</h1></div><p>Scene v1 turns a source clip, character, and cue into a deterministic render request. No account required; this workspace is tied to a secure browser cookie.</p></header>
    <div className="workspace-grid">
      <section className="editor" aria-labelledby="editor-title">
        ...
        <form className="scene-form" onSubmit={onSaveShot}>
          <fieldset disabled={!project || busy !== null}><legend>Shot and timing</legend><div className="fields two"><Field label="Shot name"><input value={draft.shotName} onChange={(e) => update("shotName",e.target.value)} maxLength={120} required /></Field><Field label="Interrupt at (seconds)"><input type="number" min="0" step="0.1" value={draft.pauseAt} onChange={(e) => update("pauseAt",e.target.value)} required /></Field></div><Field label="Source video file (uploads directly to Blob)"><input type="file" accept="video/mp4,video/quicktime,video/webm" onChange={(e) => setSourceFile(e.target.files?.[0] || null)} required={!draft.sourceVideo} /></Field>{draft.sourceVideo&&<small>Uploaded source: {draft.sourceVideo}</small>}</fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Spokescharacter</legend><Field label="Character .blend file (uploads directly to Blob)"><input type="file" accept=".blend,application/x-blender" onChange={(e) => setCharacterFile(e.target.files?.[0] || null)} required={!draft.characterAsset} /></Field><div className="fields three"><Field label="Position"><select value={draft.position} onChange={(e) => update("position",e.target.value as InterruptionDraft["position"])}><option value="foreground_left">Left</option><option value="foreground_center">Center</option><option value="foreground_right">Right</option></select></Field><Field label="Entrance"><select value={draft.entrance} onChange={(e) => update("entrance",e.target.value as InterruptionDraft["entrance"])}><option value="pop_in">Pop in</option><option value="fade_in">Fade in</option><option value="slide_left">Slide left</option><option value="slide_right">Slide right</option></select></Field><Field label="Gesture"><select value={draft.gesture} onChange={(e) => update("gesture",e.target.value as InterruptionDraft["gesture"])}><option value="shrug_and_point">Shrug + point</option><option value="wave">Wave</option><option value="nod">Nod</option><option value="talk_idle">Talk idle</option></select></Field></div><Field label="Dialogue"><textarea value={draft.dialogueText} onChange={(e) => update("dialogueText",e.target.value)} rows={3} /></Field><div className="fields two"><Field label="Voice reference"><input value={draft.voice} onChange={(e) => update("voice",e.target.value)} required /></Field><Field label="Lip-sync reference"><input value={draft.lipSync} onChange={(e) => update("lipSync",e.target.value)} required /></Field></div></fieldset>
          <fieldset disabled={!project || busy !== null}><legend>Brand and output</legend><Field label="Logo image file (uploads directly to Blob)"><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(e) => setLogoFile(e.target.files?.[0] || null)} required={!draft.logo} /></Field><div className="fields two"><Field label="Caption placement"><select value={draft.captionStyle} onChange={(e) => update("captionStyle",e.target.value as InterruptionDraft["captionStyle"])}><option value="none">None</option><option value="lower_third">Lower third</option><option value="top">Top</option><option value="center">Center</option></select></Field><label className="check"><input type="checkbox" checked={draft.captionsEnabled} onChange={(e) => update("captionsEnabled",e.target.checked)} /><span>Generate captions</span></label></div><div className="fields three"><Field label="Width"><input type="number" min="1" value={draft.width} onChange={(e) => update("width",e.target.value)} required /></Field><Field label="Height"><input type="number" min="1" value={draft.height} onChange={(e) => update("height",e.target.value)} required /></Field><Field label="FPS"><input type="number" min="1" value={draft.fps} onChange={(e) => update("fps",e.target.value)} required /></Field></div></fieldset>
      ...
      <aside className="rail"><section className="status-card"><p className="eyebrow">Flow status</p><ol><li className={project?"done":"current"}>Project {project?.name || "not created"}</li><li className={shot?"done":project?"current":""}>Shot {shot?.name || "not saved"}</li><li className={job?"done":shot?"current":""}>Render {job ? `${job.status} · ${job.id}` : "not submitted"}</li></ol>{job ? <p className="job-note">Latest: <strong>{job.status}</strong> ({job.progress}%).</p>:null}{history.length>0&&<div className="history"><strong>Render history</strong>{history.slice(0,5).map((item)=><div key={item.id}>{item.id} · {item.status} · {item.progress}%</div>)}</div>}</section>
      <section className="setup-card"><p className="eyebrow">Cloud renderer</p><h2>Deployment prerequisites</h2><p>The editor is ready; a render succeeds only after all four services are wired.</p><ul><li><strong>Blob</strong><span>Create a Vercel Blob store; set <code>BLOB_READ_WRITE_TOKEN</code>.</span></li><li><strong>Supabase</strong><span>Run <code>db/</code> SQL; set <code>SUPABASE_URL</code> and <code>SUPABASE_SERVICE_ROLE_KEY</code>.</span></li><li><strong>Queue</strong><span>Set <code>RENDER_QUEUE_URL</code> to an HTTP dispatcher accepting <code>{`{ renderJobId }`}</code>.</span></li><li><strong>Worker</strong><span>Deploy the renderer with matching <code>RENDER_WORKER_SECRET</code> and Blob/Supabase access.</span></li></ul><p className="setup-foot">Add variables in Vercel → Project Settings → Environment Variables, redeploy, then submit this saved snapshot.</p></section></aside>
    </div>
  </main>;
}
