...
def write_caption_file(path, text, start, end, style, width, height):
    """Write a deterministic libass subtitle event for the interruption window."""
    alignment = {"lower_third": 2, "top": 8, "center": 5}.get(style, 2)
    subtitle = str(text).replace("\\", "\\\\").replace("\n", "\\N").replace("{", "\\{").replace("}", "\\}")
    content = (
        "[Script Info]\n"
        "; Generated by ulo-videos\n"
        "ScriptType: v4.00+\n"
        ...
        "Style: Default,DejaVu Sans,42,&H00FFFFFF,&H00FFFFFF,&H00000000,&H99000000,0,0,0,0,100,100,0,0,4,18,0,"
        f"{alignment},48,48,60,1\n\n"
        "[Events]\n"
        "Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n"
        f"Dialogue: 0,{_ass_timestamp(start)},{_ass_timestamp(end)},Default,,0,0,0,,{subtitle}\n"
    )
...
def ffmpeg_command(source, output, trigger, width, height, fps, *, logo=None, caption_text=None, caption_style="none", caption_file=None):
    ...
    if caption_text and caption_style != "none":
        if not caption_file:
            raise ValueError("caption_file is required for fallback captions")
        escaped_caption_file = str(caption_file).replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'")
        graph.append(f"[{video}]subtitles=filename='{escaped_caption_file}'[captioned]")
        video = "captioned"
...
def render_cloud_job(job_id, callback_origin):
    ...
    try:
        ...
        caption_file = None
        if caption_text and caption_style != "none":
            caption_file = output / "captions.ass"
            write_caption_file(caption_file, caption_text, scene["trigger"]["value"], float(scene["trigger"]["value"]) + 2, caption_style, scene["output"]["width"], scene["output"]["height"])
