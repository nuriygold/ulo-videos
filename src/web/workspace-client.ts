...
export async function listRenderJobs(projectId: string | undefined, request = fetch) { return (await get<{ jobs: Array<{ id: string; status: string; progress: number; output_asset_id?: string; error_message?: string }> }>(`/api/render-jobs${projectId ? `?projectId=${encodeURIComponent(projectId)}` : ""}`, request)).jobs; }
